...
#[tauri::command]
pub async fn listen_to_official_chat(
    app: AppHandle,
    ...
    let listener_handle = tokio::spawn(async move {
        ...
        let target = FirestoreListenerTarget::new(100_u32);
        ...
        println!("📡 Listening to official chat of customer: {}", customer_id);
                        ...
                        FirestoreListenEvent::DocumentChange(ref doc_change) => {
                            if let Some(doc) = &doc_change.document {
                                    ...
                                    FirestoreDb::deserialize_doc_to::<OfficialChatMessage>(doc)
                                {
                                    ...
                                    if let Err(e) = app_clone.emit("firestore-official-chat-update", json!({
                                        "type": "added",
                                        "message": obj
                                    })) {
...
#[tauri::command]
pub async fn send_official_chat_message(
    state: State<'_, AppState>,
    customer_id: String,
    sender_id: String,
    content: String,
    ...
    // 🟢 Tambahkan dokumen customer jika belum ada
    let _ = firestore
        .fluent()
        .insert()
        .into("official_chat_customer_service")
    ...
    println!("✉️ Sent message for customer {}: {:?}", customer_id, message);
    Ok(())
}
...
#[tauri::command]
pub async fn fetch_official_chat_customers(
    state: State<'_, AppState>,
    ...
    print!("📜 Document references: {:?}", doc_refs);

    // Ubah jadi struct CustomerInfo manual
...
#[tauri::command]
pub async fn fetch_official_chat_messages(
    state: State<'_, AppState>,
    customer_id: String,
