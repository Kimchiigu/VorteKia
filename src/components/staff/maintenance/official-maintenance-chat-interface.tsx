...
export interface RideManagerInfo {
  id: string;
}
...
export function OfficialMaintenanceChatInterface({ staffId }: OfficialChatInterfaceProps) {
  const [rideManagers, setRideManagers] = useState<RideManagerInfo[]>([]);
  ...
  useEffect(() => {
    const fetchRideManagers = async () => {
      try {
        const result: RideManagerInfo[] = await invoke(
          "fetch_maintenance_chat_ride_managers"
        );
        console.log("Fetched Ride Managers:", result);
        setRideManagers(result);
      } catch (e) {
        console.error("❌ Failed to fetch ride managers:", e);
      }
    };
    fetchRideManagers();
  }, []);

  const handleSelectChat = async (rideManagerId: string) => {
    setSelectedId(rideManagerId);
    ...
    try {
      ...
      setChatMessages((prev) => ({ ...prev, [rideManagerId]: messages }));
    } catch (err) {
  ...
  useEffect(() => {
    const setupListener = async () => {
      await listen("firestore-official-chat-update", (event) => {
        ...
        const chatId = msg.sender_id === staffId ? selectedId! : msg.sender_id;
          ...
          <>
            ...
            <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
              {chatMessages[selectedId]?.map((msg) => {
                const isStaff = msg.sender_id.startsWith("MAM");
                return (
                  <div
                    key={msg.message_id}
                    className={`flex ${
